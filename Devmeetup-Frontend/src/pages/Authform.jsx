            ...
            {!forgotSent ? (
                ...
                <form onSubmit={(e) => { e.preventDefault(); setForgotSent(true); }} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-neutral-700 rounded-xl px-4 py-3 text-black placeholder-neutral-600 text-sm focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-all"
                    />
                  </div>
                  <button type="submit" className="w-full bg-amber-400 hover:bg-amber-300 text-black font-bold py-3 rounded-xl text-sm transition-all duration-200 mt-2">
                    Send reset link
                  </button>
                </form>
              </>
              ...
              <div className="text-center py-8">
                <div className="w-14 h-14 rounded-full bg-amber-400/10 border border-amber-400/30 flex items-center justify-center mx-auto mb-4">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2"><path d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0l-8-5-8 5"/></svg>
                </div>
                ...
                <button onClick={() => { setMode("signin"); setForgotSent(false); }} className="mt-6 text-sm text-amber-400 hover:text-amber-300 underline underline-offset-4 transition-colors">
                  Back to sign in
                </button>
              </div>
            )}
            ...
            <form className="space-y-4" onSubmit={handleSignIn}>
              ...
              <div>
                <div className="flex justify-between mb-1.5">
                  ...
                  <button type="button" onClick={() => { setMode("forgot"); setError(""); }} className="text-xs text-amber-500 cursor-pointer hover:text-amber-400 transition-colors">
                    Forgot password?
                  </button>
                </div>
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full border border-neutral-300 rounded-xl px-4 py-3 text-black placeholder-neutral-400 text-sm focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-all pr-11"
                  />
              ...
              <button disabled={loading} type="submit" className="w-full bg-amber-400 hover:bg-amber-300 active:bg-amber-500 text-black font-bold py-3 cursor-pointer rounded-xl text-sm transition-all duration-200 mt-2 shadow-lg shadow-amber-400/20 disabled:opacity-50">
                {loading ? "Signing in..." : "Sign in"}
              </button>
            </form>
            <p className="text-center text-sm text-neutral-500 mt-6">
              Don't have an account?{" "}
              <button onClick={() => { setMode("signup"); setError(""); }} className="text-amber-500 cursor-pointer hover:text-amber-400 font-semibold transition-colors">
                Create one
              </button>
            </p>
          </>
        ...
        {mode === "signup" && (
          <>
            ...
            <p className="text-center text-sm text-neutral-500 mt-5">
              ...
              <button onClick={() => { setMode("signin"); setError(""); }} className="text-amber-500 cursor-pointer hover:text-amber-400 font-semibold transition-colors">
                Sign in
              </button>
            </p>
          </>
