  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        date: formData.date,
        // FIX: Explicitly ensure capacity is a Number
        capacity: Number(formData.capacity),
        // FIX: Ensure location format doesn't break backend validation if it expects a clean URL or string
      ...
      <main className="flex-1 flex items-center justify-center px-6 pb-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-xl">
          ...
          <div className="flex p-1 mb-8 bg-gray-100 rounded-2xl">
            <button onClick={() => setEventType("physical")} className={`flex-1 py-3 text-sm font-bold cursor-pointer rounded-xl transition-all ${eventType === "physical" ? "bg-white shadow-sm" : "text-gray-500"}`}>Physical</button>
            <button onClick={() => setEventType("virtual")} className={`flex-1 py-3 text-sm font-bold rounded-xl cursor-pointer transition-all ${eventType === "virtual" ? "bg-white shadow-sm" : "text-gray-500"}`}>Virtual</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-bold mb-2">{eventType === "physical" ? "Venue Location" : "Meeting Link (Zoom/Google Meet)"}</label>
              <input required name="location" value={formData.location} onChange={handleChange} placeholder={eventType === "physical" ? "e.g. Eko Hotel, Lagos" : "https://zoom.us/j/..."} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:border-amber-400 transition-all" />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full py-4 mt-4 cursor-pointer font-bold text-black bg-amber-400 rounded-2xl shadow-lg shadow-amber-400/20 hover:bg-amber-500 transition-all flex items-center justify-center">
              {loading ? <div className="animate-spin h-5 w-5 border-2 border-black border-t-transparent rounded-full" /> : "Create Event"}
            </button>
          </form>
        </motion.div>
      </main>
