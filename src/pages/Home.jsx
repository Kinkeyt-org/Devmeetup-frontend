          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            {loading ? (
              <div className="flex gap-4 overflow-x-auto scrollbar-hide">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="min-w-65 h-50 rounded-2xl bg-neutral-100 dark:bg-neutral-900 animate-pulse" />
                ))}
              </div>
            ) : (
              <div className="flex md:grid md:grid-cols-3 gap-4 overflow-x-auto md:overflow-visible scrollbar-hide snap-x snap-mandatory pb-2">
                {featuredEvents.map((event) => (
                  ...
                  >
                    <img src={event.banner || event.image} alt={event.title} className="h-25 md:h-30 w-full object-cover" />
                    <div className="p-4">
                      <h3 className="font-medium">{event.title}</h3>
                      <div className="text-xs text-neutral-500 mt-2 flex flex-col gap-1">
                        <span className="flex items-center gap-1"><Calendar size={12} /> {event.event_date_human}</span>
