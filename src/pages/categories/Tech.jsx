      ...
      try {
        ...
        const filtered = allEvents.filter(e => {
          ...
          return (
            cat === "technology" ||
            cat === "tech" ||
            tags.some(t => t.toLowerCase() === "technology" || t.toLowerCase() === "tech")
          );
        });
        setEvents(filtered);
      } catch (err) {
        console.error("Failed to fetch tech events:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <section className="max-w-7xl mx-auto px-6 py-12 lg:py-20 w-full">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-neutral-500 border border-dashed rounded-4xl border-neutral-200 dark:border-neutral-800">
            <p className="text-sm">No upcoming Tech events found. Check back later!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((event) => (
